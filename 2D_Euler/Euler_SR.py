...
equations on Minkowski space in T1 symmetry.
...
def compute_char_speeds_x(prims_plus,prims_minus,K):
        global CS_x
        ...
        lam2_plus = ((-1 + K)*u1_plus - np.sqrt(K*(-1 + u1_plus**2 + u2_plus**2)*(-1 + u1_plus**2 + K*u2_plus**2)))/(-1 + K*(u1_plus**2 + u2_plus**2))
        lam2_minus = ((-1 + K)*u1_minus - np.sqrt(K*(-1 + u1_minus**2 + u2_minus**2)*(-1 + u1_minus**2 + K*u2_minus**2)))/(-1 + K*(u1_minus**2 + u2_minus**2))

        lam3_plus = ((-1 + K)*u1_plus + np.sqrt(K*(-1 + u1_plus**2 + u2_plus**2)*(-1 + u1_plus**2 + K*u2_plus**2)))/(-1 + K*(u1_plus**2 + u2_plus**2))
        lam3_minus = ((-1 + K)*u1_minus + np.sqrt(K*(-1 + u1_minus**2 + u2_minus**2)*(-1 + u1_minus**2 + K*u2_minus**2)))/(-1 + K*(u1_minus**2 + u2_minus**2))
...
def compute_char_speeds_y(prims_plus,prims_minus,K):
        global CS_y
        ...
        lam2_plus = ((-1 + K)*u2_plus - np.sqrt(K*(-1 + u1_plus**2 + u2_plus**2)*(-1 + K*u1_plus**2 + u2_plus**2)))/(-1 + K*(u1_plus**2 + u2_plus**2))
        lam2_minus = ((-1 + K)*u2_minus - np.sqrt(K*(-1 + u1_minus**2 + u2_minus**2)*(-1 + K*u1_minus**2 + u2_minus**2)))/(-1 + K*(u1_minus**2 + u2_minus**2))

        lam3_plus = ((-1 + K)*u2_plus + np.sqrt(K*(-1 + u1_plus**2 + u2_plus**2)*(-1 + K*u1_plus**2 + u2_plus**2)))/(-1 + K*(u1_plus**2 + u2_plus**2))
        lam3_minus = ((-1 + K)*u2_minus + np.sqrt(K*(-1 + u1_minus**2 + u2_minus**2)*(-1 + K*u1_minus**2 + u2_minus**2)))/(-1 + K*(u1_minus**2 + u2_minus**2))
...
def compute_flux_F1(prims_plus,prims_minus,K):

        """ Compute the fluxes in x-direction"""
        
        u1_plus = prims_plus[0] 
        u2_plus = prims_plus[1] 
        mu_plus = prims_plus[2] 
       

        Gamma2_plus = 1/(1 - u1_plus**2 - u2_plus**2)

        F1_tau_plus = (K+1)*Gamma2_plus*mu_plus*u1_plus
        F1_S1_plus = (K+1)*Gamma2_plus*mu_plus*u1_plus**2 + K*mu_plus
        F1_S2_plus = (K+1)*Gamma2_plus*mu_plus*u1_plus*u2_plus

        flux_plus = np.array([F1_tau_plus,F1_S1_plus,F1_S2_plus])


        u1_minus = prims_minus[0] 
        u2_minus = prims_minus[1] 
        mu_minus = prims_minus[2] 

        Gamma2_minus = 1/(1 - u1_minus**2 - u2_minus**2)

        F1_tau_minus = (K+1)*Gamma2_minus*mu_minus*u1_minus
        F1_S1_minus = (K+1)*Gamma2_minus*mu_minus*u1_minus**2 + K*mu_minus
        F1_S2_minus = (K+1)*Gamma2_minus*mu_minus*u1_minus*u2_minus
...
def compute_flux_F2(prims_plus,prims_minus,K):
        ...
        F1_tau_plus = (K+1)*Gamma2_plus*mu_plus*u2_plus
        F1_S1_plus = (K+1)*Gamma2_plus*mu_plus*u1_plus*u2_plus
        F1_S2_plus = (K+1)*Gamma2_plus*mu_plus*u2_plus**2 + K*mu_plus
        ...
        F1_tau_minus = (K+1)*Gamma2_minus*mu_minus*u2_minus
        F1_S1_minus = (K+1)*Gamma2_minus*mu_minus*u1_minus*u2_minus
        F1_S2_minus = (K+1)*Gamma2_minus*mu_minus*u2_minus**2 + K*mu_minus
...
########################################################################################################
def T1_Euler(t,cons,step_sizes,Ngz,K):
    ...
    char_speed_x = compute_char_speeds_x(prims_plus_x,prims_minus_x,K)

    char_speed_y = compute_char_speeds_y(prims_plus_y,prims_minus_y,K)
    ...
    flux_plus_F1, flux_minus_F1 = compute_flux_F1(prims_plus_x,prims_minus_x,K)

    flux_plus_F2, flux_minus_F2 = compute_flux_F2(prims_plus_y,prims_minus_y,K)
...
def evolve_system(yinit,t0,tend,spatial_steps,rk,rhs,BCs,K,Nghosts):
    ...
    while t0<tend:
        ...
        # dt = 0.4*dx
        if dt > 0.4*dx:
            # print(CFL*np.min([dx/CS_x,dy/CS_y]))
            # print(CFL*dx)
            # print("Here!")
            dt = 0.4*dx
...
lam2_x = ((-1+K)*u1 + np.sqrt(K*(1-u1**2-u2**2)*(1-u1**2 - K*u2**2)))\
                      /(-1 + K*(u1**2 + u2**2))
lam3_x = ((-1+K)*u1 - np.sqrt(K*(1-u1**2-u2**2)*(1-u1**2 - K*u2**2)))\
                      /(-1 + K*(u1**2 + u2**2))
...
lam2_y = ((-1+K)*u2 + np.sqrt(K*(1-u1**2-u2**2)*(1-K*u1**2 - u2**2)))\
                      /(-1 + K*(u1**2 + u2**2))
lam3_y = ((-1+K)*u2 - np.sqrt(K*(1-u1**2-u2**2)*(1-K*u1**2 - u2**2)))\
                      /(-1 + K*(u1**2 + u2**2))
...
# Evolve System
############################################################################
rhs = lambda t,y: T1_Euler(t,y,spatial_steps,Nghosts,K)
